...
def choice_prompt_func(prompt_func: Callable[[], str], choices: Sequence[str]) -> Callable[[], str]:
    ...
    '''
    return lambda: '%s %s: ' % (pretty_text(prompt_func()), choices)

def pretty_echo(message, color='cyan'):
    # wrapped_msg = textwrap.fill(message, width=120)
    colored_text = colored(message, color)
    click.echo(colored_text)

def pretty_text(message, color='cyan'):
    # wrapped_msg = textwrap.fill(message, width=120)
    colored_text = colored(message, color)
    return colored_text