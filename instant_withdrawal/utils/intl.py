...
from instant_withdrawal.utils.file_handling import (
    resource_path,
)
...
def load_text(params: List[str], file_path: str = '', func: str = '', lang: str = '', color='cyan') -> str:
    '''
    ...
    rel_path_list = file_path_list[file_path_list.index('instant_withdrawal') + 1:]
    json_path = resource_path(os.path.join(
        INTL_CONTENT_PATH, lang, *rel_path_list))

    try:
        # browse json until text is found
        with open(json_path) as f:
            text_dict = json.load(f)
            return pretty_text(_get_from_dict(text_dict, [func] + params), color)
...
def fuzzy_reverse_dict_lookup(text: str, options: Mapping[str, Sequence[str]]) -> str:
    ...
    '''
    reverse_lookup_dict = {value: key for key,
                           values in options.items() for value in values}
